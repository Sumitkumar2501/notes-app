...
import { Pin, Archive as ArchiveIcon, Trash2, RefreshCcw, XCircle, Edit2, Clock } from "lucide-react";
...
export const NotesCard = ({ id, title, text, isPinned, deletedAt, isTrash, createdAt, updatedAt }) => {
    ...
    return (
        <>
            <div className="glass-card p-3 sm:p-5 md:p-6 w-full sm:w-[300px] flex flex-col group relative overflow-hidden animate-fade-in transition-all duration-200 hover:shadow-xl" key={id} style={{ willChange: 'transform', minHeight: '200px' }}>
                {/* Visual Indicators */}
                ...
                <div className="flex justify-between items-start border-b pb-2 sm:pb-4 mb-2 sm:mb-4 gap-2" style={{ borderColor: 'var(--border-primary)' }}>
                    <p className="font-semibold text-sm sm:text-lg break-words flex-1 pr-1 leading-tight" style={{ color: 'var(--text-primary)' }}>{title}</p>
                    {
                        !isNotesInArchive && !isTrash && (
                            <button 
                                onClick={() => onPinClick(id)} 
                                className={`flex-shrink-0 p-2 sm:p-2 rounded-lg transition-all duration-200 hover:opacity-80 active:scale-95 ${isPinned ? 'bg-yellow-100 text-yellow-600' : ''}`}
                ...
                <div className="flex flex-col flex-grow">
                    ...
                    {/* Timestamp Info */}
                    {createdAt && (
                        <div className="mb-3 sm:mb-4 flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                            <Clock size={12} />
                            ...
                            {isTrash ? (
                                <>
                                    <button 
                                        ...
                                        className="p-2 sm:p-2 rounded-lg transition-all duration-200 hover:opacity-80 active:scale-95 touch-manipulation"
                                        style={{ color: 'var(--text-tertiary)', backgroundColor: 'rgba(16, 185, 129, 0.1)', minHeight: '40px', minWidth: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                        title="Restore"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="p-2 sm:p-2 rounded-lg transition-all duration-200 hover:opacity-80 active:scale-95 touch-manipulation"
                                        style={{ color: 'var(--text-tertiary)', backgroundColor: 'rgba(239, 68, 68, 0.1)', minHeight: '40px', minWidth: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                ...
                                <>
                                    <button 
                                        ...
                                        className="p-2 sm:p-2 rounded-lg transition-all duration-200 hover:opacity-80 active:scale-95 touch-manipulation"
                                        style={{ color: 'var(--text-tertiary)', backgroundColor: 'rgba(94, 74, 65, 0.1)', minHeight: '40px', minWidth: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                        title="Edit"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className={`p-2 sm:p-2 rounded-lg transition-all duration-200 hover:opacity-80 active:scale-95 touch-manipulation ${isNotesInArchive ? 'bg-blue-100 text-blue-600' : ''}`}
                                        style={!isNotesInArchive ? { color: 'var(--text-tertiary)', backgroundColor: 'rgba(59, 130, 246, 0.1)', minHeight: '40px', minWidth: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' } : { minHeight: '40px', minWidth: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
